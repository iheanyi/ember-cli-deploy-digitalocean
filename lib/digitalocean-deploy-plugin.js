...
var NPMInstallTask = require('./commands/npm-install');
var RemoteCommandTask = require('./commands/remote-command');
var UploadAppTask = require('./commands/upload-app');
var UploadFileTask = require('./commands/upload-file');
...
module.exports = DeployPlugin.extend({
  ...
  willUpload: function(context) {
    ...
    var installDependenciesCommand = 'sudo apt-get update -y; sudo apt-get upgrade -y; sudo apt-get install -y nginx gcc build-essential; rm /etc/nginx/sites-enabled/default;';
    var provisionTask = new RemoteCommandTask({
      log: this.log.bind(this),
      command: installDependenciesCommand,
      conn: this.conn.bind(this),
      ipAddress: this.config.ipAddress,
    });

    return npmInstallTask.run()
    .then(() => {
      return provisionTask.run().then(() => {
        this.log("Done provisioning droplet.");
      });
    }).catch(err => {
      throw err;
    });
  ...
  didUpload: function(context) {
    ...
    var uploadFastbootConfTask = new UploadFileTask({
    ...
    var restartServicesCommand = "sudo service nginx restart; sudo service fastboot restart";
