...
module.exports = DeployPlugin.extend({
  ...
  willUpload: function(context) {
    ...
    return new Promise((resolve, reject) => {
      var conn = this.conn;
      conn.on('ready', () => {
        conn.exec('sudo apt-get update -y; sudo apt-get upgrade -y; sudo apt-get install -y nginx gcc build-essential', (err, stream) => {
          if (err) throw err;
          stream.on('data', (data) => {
  ...
  upload: function(context) {
    ...
    return new Promise((resolve, reject) => {
      scpClient.scp(context.distDir, scpConfig, (err) => {
        ...
        fileClient.upload('./node_modules/ember-cli-deploy-digitalocean/templates/nginx.conf', '/etc/nginx/sites-enabled/default', function(err) {
          if (err) {
            this.log(err);
            throw err;
          }
