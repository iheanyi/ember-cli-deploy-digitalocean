...
module.exports = DeployPlugin.extend({
  defaultConfig: {
    ...
    privateKeyPath: process.env.PRIVATE_KEY_PATH || process.env.PRIVATE_KEY_DIR || '~/.ssh/id_rsa',
    dropletUsername: process.env.DROPLET_USERNAME || 'root',
    dropletPassword: process.env.DROPLET_PASSWORD,
    passphrase: process.env.SSH_KEY_PASSPHRASE || process.env.PASSPHRASE,
  ...
  requiredConfig: ['environment', 'dropletUsername'],
  ...
  configure: function() {
    ...
    this.scpConfig = Object.assign({}, this.sshConfig, {
