...
module.exports = {
  ...
  run: function() {
    ...
    return Promise.all([dropletsPromise, regionsPromise, keysPromise, dropletSizesPromise]).then((values) => { 
      ...
      .then(() => {
        console.log("Done provisioning the Droplet!");
      });
    });
  },
  provisionDroplet: function() {
    var PRIVATE_KEY = process.env.PRIVATE_KEY_DIR;
    if (!PRIVATE_KEY) {
      throw new Error('Environment Variable PRIVATE_KEY_DIR is not set, please set it to the path of your SSH Key.');
    }

    var PASSPHRASE = process.env.PASSPHRASE;
    if (!PASSPHRASE) {
      throw new Error('Environment Variable PASSPHRASE is not set, please set it to the passphrase of your SSH key.');
    }

    const conn = new SSHClient();

    const provisionScriptPath = path.join('./node_modules', 'ember-cli-deploy-digitalocean', 'scripts', 'provision-droplet.sh');

    const provisionCommand = fs.readFileSync(provisionScriptPath, {encoding: 'utf8'});

    var provisionTask = new RemoteCommandTask({
      log: console.log.bind(this),
      command: provisionCommand,
      conn: conn,
      ipAddress: this.dropletIp
    });

    return provisionTask.run();
  },
  uploadNginxConf: function() {
    var millseconds = (new Date).getTime();

    var appName = 'emberApp-' + milliseconds;
    var nginxConfig = {
      host: this.dropletIp,
      port: 22,
    ...
    var nginxClient = new SCPClient.Client(nginxConfig);

    var nginxPath = path.join('./node_modules', 'ember-cli-deploy-digitalocean', 'templates', 'nginx.conf');
    var uploadNginxConfigTask = new UploadFileTask({
      localFile: nginxPath,
      remoteFile: '/etc/nginx/nginx.conf',
      scpClient: nginxClient,
      log: console.log.bind(this)
    });
  ...
  createDropletIfNeeded: function() {
    ...
    if (answers.dropletName !== undefined) {
      ...
      .then(droplet => {
        ...
        this.dropletIp = droplet.networks.v4[0].ip_address;
      });
    }

    this.dropletIp = answers.droplet.networks.v4[0].ip_address;
